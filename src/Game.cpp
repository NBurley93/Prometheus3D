...
void Game::Init() {
	...
	for (int i = 1; i <= 17; i++) {
		SoundManager::Get()->addSFX("./data/sounds/footsteps/concrete_ct_" + std::to_string(i) + ".wav");
	}
	...
	mWindow = new Window();
	mWindow->CreateWindow(920, 600, "Prometheus");
	...
	CommandConsole::start(920, 600);
	CommandConsole::registerCommand("glVersion", con_glVersion, "- Prints the OpenGL context version information");
	CommandConsole::registerCommand("version", con_version, "- Prints engine version");
	CommandConsole::registerCommand("tShadows", con_toggleShadow, "- Toggles shadow mapping");
	CommandConsole::registerCommand("tWireframe", con_toggleWireframe, "- Toggles wireframe");
	CommandConsole::registerCommand("debugShadows", con_debugShadows, "- Toggles debug view for shadow mapping");
	CommandConsole::registerCommand("tNormalMapping", con_toggleNormalMapping, "- Toggles use of normal maps (STUB - DOES NOTHING)");
	CommandConsole::registerCommand("tAnimations", con_toggleAnimations, "- Toggles use of animations");
	CommandConsole::registerCommand("modval", con_modVal, "- Modify a global value, use listval to show modifiable values");
	CommandConsole::registerCommand("listval", con_listVal, "- Lists all modifiable values");
	...
	mFPArms->GetMesh(0)->AddTexture("diffuse", "./data/textures/fp/diffuse_torch.tga");

	mWell = new Model();
	mWell->Load("./data/models/well.dae");
	mWell->GetMesh(0)->AddTexture("diffuse", "./data/textures/well/well_diffuse.tga");
	mWell->GetMesh(0)->AddTexture("specular", "./data/textures/well/well_specular.tga");
	mWell->GetMesh(1)->AddTexture("diffuse", "./data/textures/well/water_diffuse.tga");

	mCutout = new Model();
	mCutout->Load("./data/models/cardboard_cutout.dae");
	mCutout->GetMesh(0)->AddTexture("diffuse", "./data/textures/cardboard/house.tga");
	mCutout->GetMesh(1)->AddTexture("diffuse", "./data/textures/cardboard/cardboard_edge.tga");
	...
	mModel->GetMesh(0)->AddTexture("diffuse", "./data/textures/cyborg/diffuse.tga");
	...
	//Camera definitions
	mCamera = new Camera();
	mCamera->defineFOV(45.0f);
	mCamera->init(920.0f, 600.0f);
	...
	//Opengl logo sprite
	mRenderer->CacheSprite(TextureManager::Get()->getTexture("./data/textures/opengl_logo.tga"));
	...
	mWorld->GetMesh(0)->AddTexture("diffuse", "./data/textures/brick/diffuse.tga");
	mWorld->GetMesh(0)->AddTexture("specular", "./data/textures/brick/specular.tga");
	...
	for (GLuint i = 0; i < 6; ++i) {
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_DEPTH_COMPONENT, SHADOW_FACTOR, SHADOW_FACTOR, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
	}
...
void Game::Run() {
	//Fetch the performance counter from SDL to help construct delta-time and loop
	mLastFrame = 0.0f;
	int frameTicks = 0;
...
void Game::Update() {
	...
	if (mInput->isKeyPressed(SDLK_BACKQUOTE)) {
		Globals::Get()->input_text = "";
		CommandConsole::toggleVisiblility(); //Console mechanically works, still need to adapt it for OpenGL
	}
	...
	if ((mInput->isKeyDown(SDLK_BACKSPACE) && Globals::Get()->input_text.length() > 0)) {
	...
	LightManager::Get()->Lights[1].direction = glm::vec4(pp.x, pp.y + 2.0f, pp.z, 0.0f);
...
void Game::Render() {
	...
	glUniformMatrix4fv(shdr->getUniformLocation("view"), 1, false, glm::value_ptr(mCamera->getMatrix("view")));
	glUniformMatrix4fv(shdr->getUniformLocation("proj"), 1, false, glm::value_ptr(mCamera->getMatrix("projection")));
	...
	glUniform1f(shdr->getUniformLocation("numSamples"), Globals::Get()->shadowPCFSamples);
	glUniform1i(shdr->getUniformLocation("uShadows"), Globals::Get()->uShadows);
	glUniform1i(shdr->getUniformLocation("uDebug"), Globals::Get()->uDebugShadows);
	...
	glUniform1f(shdr->getUniformLocation("far_plane"), far);
	glUniform1f(shdr->getUniformLocation("numSamples"), Globals::Get()->shadowPCFSamples);
	glUniform1i(shdr->getUniformLocation("uAnimation"), Globals::Get()->uAnimation);
	glUniform1i(shdr->getUniformLocation("uShadows"), Globals::Get()->uShadows);
	glUniform1i(shdr->getUniformLocation("uDebug"), Globals::Get()->uDebugShadows);
	...
	float mod = 35.0f;
	mModel->Translate(glm::vec3(ppos.x - (plook.x * mod), ppos.y, ppos.z - (plook.z * mod)));
	mModel->Rotate(glm::radians(mCamera->get_hAngle() - 90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	mModel->Rotate(glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
	
	mModel->Draw(*shdr, true, true);
...
void Game::RenderShadowMapping() {
	...
	for (int i = 0; i < MAX_LIGHTS; i++) {
		...
		if (mLight.enabled) {
			...
			glm::vec3 lightPos = glm::vec3(mLight.direction.x, mLight.direction.y, mLight.direction.z);
			...
			//Uniforms
			for (GLuint i = 0; i < 6; ++i) {
				glUniformMatrix4fv(shdr->getUniformLocation("shadowMatrices[" + std::to_string(i) + "]"), 1, GL_FALSE, glm::value_ptr(shadowTransforms[i]));
			}
			glUniform1f(shdr->getUniformLocation("far_plane"), far);
...
void Game::RenderFPArms() {
	...
	shdr->use();
		...
		glUniformMatrix4fv(shdr->getUniformLocation("view"), 1, false, glm::value_ptr(mCamera->getMatrix("view")));
		glUniformMatrix4fv(shdr->getUniformLocation("proj"), 1, false, glm::value_ptr(mCamera->getMatrix("projection")));
		glClear(GL_DEPTH_BUFFER_BIT); //Clear the depth buffer so that the fp arms are always drawn on top
		mFPArms->Draw(*shdr);
	shdr->unuse();
}
...
void Game::RenderGUI() {
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); //Guarentee that normal rendering, not wireframe is set

	//Draw 2D gui elements
	glDisable(GL_DEPTH_TEST); //Wouldn't make much sense for 2D ortho elements to z-fight
	mRenderer->RenderSprite(TextureManager::Get()->getTexture("./data/textures/opengl_logo.tga"), *mCamera, (920 - 8) - 256, 8, 256, 128, 0.4f);
	...
	glUniformMatrix4fv(ShaderManager::Get()->getShdr("gui_text")->getUniformLocation("projection"), 1, false, glm::value_ptr(mCamera->getMatrix("ortho")));

	testFont.RenderText(*ShaderManager::Get()->getShdr("gui_text"), "CS465 Final Project", 8.0f, 24.0f, 1.0f);
	testFont.RenderText(*ShaderManager::Get()->getShdr("gui_text"), "FPS: " + std::to_string((int)(1000 / mDT)) + " (Ms/s: " + std::to_string(mDT) + ")", 8.0f, 48.0f, 1.0f);
	//testFont.RenderText(*ShaderManager::Get()->getShdr("gui_text"), "(" + std::to_string(p.x) + "," + std::to_string(p.y) + "," + std::to_string(p.z) + ")", 8.0f, 72.0f, 1.0f);

	ShaderManager::Get()->getShdr("gui_text")->unuse();

	CommandConsole::drawConsole(*mCamera, *mInput, consoleFont); //Draw the command console, still needs to be adapted for shader pipeline
...
void con_glVersion(std::string args) {
	...
	glslStr = reinterpret_cast<const char*>(glGetString(GL_SHADING_LANGUAGE_VERSION));
...
void con_version(std::string args) {
	CommandConsole::runCommand("println Version: " + std::to_string(VER_MAJOR) + "." + std::to_string(VER_MINOR) + "r" + std::to_string(B_REVISION) + " build" + std::to_string(BUILD_NUM) + " " + BUILD_DT);
}
...
void con_modVal(std::string args) {
	...
	input = args.substr(args.find_first_of(" ") + 1, args.length() - args.find_first_of(" "));
	...
	if (good) {
		...
		CommandConsole::runCommand("println The value '" + var + "' is either invalid, or was provided an invalid input");
	}
}
...
void Game::playRandFootstep() {
	...
	SoundManager::Get()->playSFX("./data/sounds/footsteps/concrete_ct_" + std::to_string(rNumber) + ".wav");
}